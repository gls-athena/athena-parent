 ...
 * 树节点解析器
 * <p>
 * 实现Hutool树形结构的节点解析器接口，用于将实现了{@link TreeNode}接口的实体对象
 * 转换为{@link Tree}树形结构节点。
 *
 * @param <T> 树节点类型，必须实现{@link TreeNode}接口
 * @author george
 * @see NodeParser
 * @see TreeNode
 */
public class TreeNodeParser<T extends TreeNode> implements NodeParser<T, Long> {

    /**
     * 解析源对象为树节点
     * <p>
     * 将实现了{@link TreeNode}接口的源对象解析为Hutool的{@link Tree}节点，
     * 包括设置基本属性（ID、父节点ID、名称、权重）和扩展属性。
     *
     * @param object   源数据对象，实现{@link TreeNode}接口
